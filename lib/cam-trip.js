...
export const API_ENDPOINTS = {
  provinces: {
    ...
    getById: (id) => `${API_BASE_URL}/provinces/${encodeURIComponent(id)}`,
  },
  attractions: {
    getAll: `${API_BASE_URL}/attractions`,
    getById: (id) => `${API_BASE_URL}/attractions/${encodeURIComponent(id)}`,
  },
};
...
// inside app/api/provinces/[id]/route.js against the teacher CamTrip API.
    ...
    for (const item of candidates) {
      ...
      selected.push({
        ...
        image: localDestinationImage(
          "attraction",
          item.id,
          item.imageUrls?.find((image) => typeof image === "string" && image) ||
